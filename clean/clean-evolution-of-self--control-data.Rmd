...
3. Import the CSV from `../data/csv/subj_info.csv`
4. Make a new column for task responses as 'Y' if '*' and 'N' if '-'.
5. Remove the old columns with '*' and '-' as input.
6. Merge cog_task_avg_score and subj_info together by Common.Name.
...
```{r export-merged}
write_csv(merged, path = "../data/cleaned/cog_score_and_sub_info.csv")
```
